...
def run(fileread, filewrite):
    global level_set
    if fileread == "MiniCosmos.txt":
        level_set = "minicosmos"
    else:
        level_set = "microcosmos"
    fulltest, list_level = read_file(fileread)
    count = 0
    while True:
        # array to take step to run in game
        A_star_path = []
        bfs_path = []

        global robot, walls, box, storage, visited_Moves, queue, dequeue, check, width, current_level
        ...
        current_level = int(level_choose)
        runtime = False    # check the game completed or not
        # Initialize Level
